 ...
 *
 *  Spanish UI variant: all player-facing strings translated to Spanish.
 *  Logic is identical to robotfindskitten.c. Pair with nki/espanol.nki for a
 *  fully Spanish game, e.g.:  robotfindskitten.es -f nki/espanol.nki
 */
...
static void do_read_messages ( char *dname ) {
	...
	while ( ( dent = readdir ( dir ) ) ) {
		...
		if ( ( fname = malloc ( len ) ) ==  NULL ) {
			(void) fprintf ( stderr, "No se pudo reservar memoria para los mensajes.\n" );
			exit ( EXIT_FAILURE );
		} else {
...
static void read_messages(void) {
	...
	if ( home_dir ) {
		...
		if ( ! ( user_nki_dir = malloc ( user_nki_len ) ) ) {
			(void) fprintf ( stderr, "No se pudo reservar memoria para el directorio NKI del usuario.\n" );
			exit ( EXIT_FAILURE );
		}
...
static void init ( unsigned int num ) {
	...
	if ( ( ( state.lines - HEADSIZE - FRAME ) * state.cols ) < (int) ( num + 2 ) ) {
		(void) endwin();
		(void) fprintf ( stderr, "¡La pantalla es demasiado pequeña para todos los objetos!\n" );
		exit ( EXIT_FAILURE );
	}
...
static void handle_resize(void) {
	...
	if (xbound >= COLS - FRAME*2 || ybound >= HEADSIZE + LINES - FRAME*2) {
		(void) endwin();
		(void) fprintf(stderr,
			"Has aplastado la simulación. Y a robot. Y a gatito.\n");
		exit(EXIT_FAILURE);
	}
...
static void instructions(void) {
	...
	(void) printw ( "robotfindskitten %s\n", PACKAGE_VERSION );
	(void) printw (
"Del ilustre Leonard Richardson (C) 1997, 2000\n"\
"Escrito originalmente para el concurso robotfindskitten de Nerth Pork\n\n"\
"En este juego, tú eres robot (#). Tu tarea es encontrar a gatito. Esta tarea\n"\
"se complica por la existencia de varias cosas que no son gatito. Robot debe\n"\
"tocar los objetos para determinar si son gatito o no. El juego termina cuando\n"\
"robotencuentragatito. Como alternativa, puedes terminar el juego pulsando la\n"
"tecla q o un clásico Ctrl-C de toda la vida.\n\n"\
...
static void play_animation ( bool fromright ) {
	...
	chtype kitty;
#define WIN_MESSAGE	"¡Has encontrado a gatito! ¡Bien hecho, robot!"
	...
	while ( ( ch = getch() ) != 0 ) {
		...
		switch ( ch ) {
			...
			case KEY_RESIZE:
				...
				break;
	                default:
	                        message ( "Entrada no válida: usa las teclas"\
					" de dirección o q." );
	                        break;
		}
		...
		switch ( test ( y, x, &bnum ) ) {
			...
			case BBOGUS:
				...
				break;
			default:
				message ( "Vaya, eso fue inesperado..." );
				break;
		}
	}
}
...
int main ( int argc, char **argv ) {
	...
	/* Honour the user's locale so (n)curses renders multibyte UTF-8
	   characters (accents, ñ, ¡, ¿ etc.) correctly instead of as blank
	   cells. */
	...
	while ((option = getopt(argc, argv, "n:s:f:Vh")) != -1) {
		...
		nbogus = atoi ( optarg );
		if ( nbogus <= 0 ) {
			(void) fprintf ( stderr, "El argumento debe ser positivo.\n" );
			exit ( EXIT_FAILURE );
		}
		break;
	    case 's':
	    ...
	    case '?':
	    default:
		(void) printf("uso: %s [-f archivo-nki] [-n num] [-s semilla] [-V]\n", argv[0]);
		exit(EXIT_SUCCESS);
	    }
	}
	...
	if (state.num_messages == 0) {
		(void) fprintf ( stderr, "No se encontraron NKIs.\n" );
		exit ( EXIT_FAILURE );
	}
	...
	if ( nbogus > (int)state.num_messages ) {
		(void) fprintf ( stderr, "Solo hay %u NKIs disponibles (se solicitaron %d).\n", state.num_messages, nbogus );
		exit ( EXIT_FAILURE );
	} else {
