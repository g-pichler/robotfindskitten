 ...
 *
 *  German UI variant: all player-facing strings translated to German.
 *  Logic is identical to robotfindskitten.c. Pair with nki/deutsch.nki for a
 *  fully German game, e.g.:  robotfindskitten.de -f nki/deutsch.nki
 */
...
static void do_read_messages ( char *dname ) {
	...
	while ( ( dent = readdir ( dir ) ) ) {
		...
		if ( ( fname = malloc ( len ) ) ==  NULL ) {
			(void) fprintf ( stderr, "Speicher für die Nachrichten konnte nicht reserviert werden.\n" );
			exit ( EXIT_FAILURE );
		} else {
...
static void read_messages(void) {
	...
	if ( home_dir ) {
		...
		if ( ! ( user_nki_dir = malloc ( user_nki_len ) ) ) {
			(void) fprintf ( stderr, "Speicher für das Benutzer-NKI-Verzeichnis konnte nicht reserviert werden.\n" );
			exit ( EXIT_FAILURE );
		}
...
static void init ( unsigned int num ) {
	...
	if ( ( ( state.lines - HEADSIZE - FRAME ) * state.cols ) < (int) ( num + 2 ) ) {
		(void) endwin();
		(void) fprintf ( stderr, "Der Bildschirm ist zu klein für alle Objekte!\n" );
		exit ( EXIT_FAILURE );
	}
...
static void handle_resize(void) {
	...
	if (xbound >= COLS - FRAME*2 || ybound >= HEADSIZE + LINES - FRAME*2) {
		(void) endwin();
		(void) fprintf(stderr,
			"Du hast die Simulation zerquetscht. Und Roboter. Und Kätzchen.\n");
		exit(EXIT_FAILURE);
	}
...
static void instructions(void) {
	...
	(void) printw ( "robotfindskitten %s\n", PACKAGE_VERSION );
	(void) printw (
"Vom erlauchten Leonard Richardson (C) 1997, 2000\n"\
"Ursprünglich geschrieben für den robotfindskitten-Wettbewerb von Nerth Pork\n\n"\
"In diesem Spiel bist du Roboter (#). Deine Aufgabe ist es, Kätzchen zu finden.\n"\
"Erschwert wird das durch allerlei Dinge, die kein Kätzchen sind. Roboter muss\n"\
"die Dinge berühren, um festzustellen, ob sie Kätzchen sind oder nicht. Das\n"\
"Spiel endet, sobald roboterfindetkätzchen. Alternativ kannst du das Spiel mit\n"
"der Taste q oder einem guten alten Strg-C beenden.\n\n"\
...
static void play_animation ( bool fromright ) {
	...
	chtype kitty;
#define WIN_MESSAGE	"Du hast Kätzchen gefunden! Gut gemacht, Roboter!"
	...
	while ( ( ch = getch() ) != 0 ) {
		...
		switch ( ch ) {
			...
			case KEY_RESIZE:
				...
				break;
	                default:
	                        message ( "Ungültige Eingabe: Benutze die"\
					" Richtungstasten oder q." );
	                        break;
		}
		...
		switch ( test ( y, x, &bnum ) ) {
			...
			case BBOGUS:
				...
				break;
			default:
				message ( "Tja, das war unerwartet ..." );
				break;
		}
	}
}
...
int main ( int argc, char **argv ) {
	...
	/* Honour the user's locale so (n)curses renders multibyte UTF-8
	   characters (umlauts etc.) correctly instead of as blank cells. */
	...
	while ((option = getopt(argc, argv, "n:s:f:Vh")) != -1) {
		...
		nbogus = atoi ( optarg );
		if ( nbogus <= 0 ) {
			(void) fprintf ( stderr, "Das Argument muss positiv sein.\n" );
			exit ( EXIT_FAILURE );
		}
		break;
	    case 's':
	    ...
	    case '?':
	    default:
		(void) printf("Aufruf: %s [-f NKI-Datei] [-n Anzahl] [-s Startwert] [-V]\n", argv[0]);
		exit(EXIT_SUCCESS);
	    }
	}
	...
	if (state.num_messages == 0) {
		(void) fprintf ( stderr, "Keine NKIs gefunden.\n" );
		exit ( EXIT_FAILURE );
	}
	...
	if ( nbogus > (int)state.num_messages ) {
		(void) fprintf ( stderr, "Es sind nur %u NKIs verfügbar (angefordert wurden %d).\n", state.num_messages, nbogus );
		exit ( EXIT_FAILURE );
	} else {
